...
// 1 STOCK NUMBER
// 1 STYLE NUMBER
// 1 SPECIFIC SIZE. DIFFERET PAIRS
// 1 DIFFERENT PRIZE
...
void shoe_record(FILE *fp)
{
    ...
    for (int i = 0; i < 12; i++) 
    {
        fprintf(fp_append, "%.2f ", invent.price[i]);
    }
    fprintf(fp_append, "\n");
...
//CHECK INDIVIDUAL SIZE
    ...
    while (fscanf(fp, "%d %d ", &invent.stockNumber, &invent.styleNumber) == 2)
        ...
        if (invent.stockNumber == tempstockNumber && invent.styleNumber == tempstyleNumber && invent.pairs[sizeIndex] > 0)
        {
            ...
            printf("\n\n\tSize %d", sizeIndex + 3); // Adjust the sizeIndex for display
            printf("\n\tNumber of pairs: %d", invent.pairs[sizeIndex]);
            printf("\n\tPrice: %.2f", invent.price[sizeIndex]);
            break;
        }
    }
...
void shoe_updateprice(FILE *fp)
{
   ...
   FILE *tempFile = fopen("temp_inventory.txt", "w");
    ...
    while (fscanf(fp, "%d %d ", &invent.stockNumber, &invent.styleNumber) == 2)
        ...
        {
            found = 1;
            printf("\n\tEnter new price for size %d: ", sizeIndex + 3);
            scanf("%f", &newPrice);

            // Find the index to update the price (assuming there are 12 elements in the 'invent.price' array)
            int priceIndex = 0;
            for (int i = 0; i < 12; i++)
            {
                if (invent.price[i] > 0) 
            ...
            invent.price[sizeIndex] = newPrice;
        }
        ...
        for (int i = 0; i < 12; i++) 
        {
            fprintf(tempFile, "%.2f ", invent.price[i]);
        }
        fprintf(tempFile, "\n");
        }
...
void shoe_delete(FILE *fp) 
{
    ...
    switch (choice) {
        case 1:
            {
                ...
                if (confirm == 'Y' || confirm == 'y') {
                    fclose(fp); // Close the file before deleting it
                    fp = fopen("inventory.txt", "w"); // Reopen the file in write mode, which truncates the file
                    if (fp == NULL) {
                        printf("Failed to create/open the file.");
                        return;
                    }
        ...
        case 2:
            ...
            int foundRecords = 0; // Variable to track the number of found records
            int deletedRecords = 0; 

            for (int i = 0; i < numRecordsToDelete; i++) 
            {
                int tempstockNumber, tempstyleNumber;
                printf("\n\tRecord %d:", i + 1);
                printf("\n\tEnter stock number: ");
                ...
                while (fscanf(fp, "%d %d ", &invent.stockNumber, &invent.styleNumber) == 2) {
                    ...
                    if (invent.stockNumber == tempstockNumber && invent.styleNumber == tempstyleNumber) {
                    foundRecords++;
                    if (foundRecords <= numRecordsToDelete) {
                        deletedRecords++;
                        continue; 
                    }
                }
		...
		do
		{
			...
			switch(choice)
			{
                ...
                printf("\n\t----------------------------\n");
					...
					{
						shoe_display(fp);
						printf("\n\n\t\tDo you want to display a specific record again (Y | N)?: ");
						scanf(" %c", &tryagain);
						while(getchar() != '\n')
							continue;
					}while(tryagain == 'Y' || tryagain == 'y');
                ...
                printf("\n\t----------------------------");
					...
					{
						shoe_allrec(fp);
						printf("\n\n\t\tDo you want to display all record again (Y | N)?: ");
						scanf(" %c", &tryagain);
						while(getchar() != '\n')
							continue;
					}while(tryagain == 'Y' || tryagain == 'y');
