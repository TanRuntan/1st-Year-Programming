...
void shoe_display(FILE *fp)
{
    ...
    fseek(fp, 0, SEEK_SET); // Rewind the file pointer to the beginning of the file
    ...
    while (fscanf(fp, "%d %d ", &invent.stockNumber, &invent.styleNumber) == 2)
    {
        ...
        // If the record is not found, move to the next line in the file
        fscanf(fp, "%*[^\n]");
    }
    ...
    switch (choice) {
        ...
        case 2:
            ...
            for (int i = 0; i < numRecordsToDelete; i++) 
            {
                ...
                while (fscanf(fp, "%d %d", &invent.stockNumber, &invent.styleNumber) == 2) 
                {
                    ...
                    if (invent.stockNumber == tempstockNumber && invent.styleNumber == tempstyleNumber) {
                        found = 1; // Mark the record as found
                        foundRecords++; // Increment the count of found records
                        // Skip reading the rest of the record and move to the next line
                        fscanf(fp, "%*[^\n]");
                    } else {
                        // Write the record to the temporary file
                        fprintf(tempFile, "%d %d ", invent.stockNumber, invent.styleNumber);
                ...
                if (found) {
                    deletedRecords++; // Increment the count of deleted records
                }
        ...
        case 3:
            ...
            if (confirm == 'Y' || confirm == 'y') {
                ...
                // Set the file pointer to NULL after closing the file
                fp = NULL;
            } else {
                printf("\n\t\tOPERATION CANCELLED.");
            }
		...
		do
		{
			...
			switch(choice)
			{
                ...
                printf("\n\t----------------------------");
					...
					{
						shoe_display(fp);
						printf("\n\t\tDo you want to display specific record again (Y | N)?: ");
						scanf(" %c", &tryagain);
						while(getchar() != '\n')
							continue;
					}while(tryagain == 'Y' || tryagain == 'y');
