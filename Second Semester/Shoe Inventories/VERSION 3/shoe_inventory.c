...
void shoe_delete(FILE *fp) 
{
    ...
    switch (choice) {
        ...
        case 2:
            ...
            int deletedRecords = 0; // Variable to track the number of deleted records
            ...
            for (int i = 0; i < numRecordsToDelete; i++) 
            {
                ...
                while (fscanf(fp, "%d %d ", &invent.stockNumber, &invent.styleNumber) == 2) {
                    ...
                    if (foundRecords <= numRecordsToDelete) {
                        deletedRecords++;
                        continue; // Skip writing this record to the temporary file (effectively deleting it)
                    }
                }
